...
11. **evaluation / confidence attestations** — `src/evaluation.js` + a `validateEvaluation` validator in `src/schema.js`: an attestation can now carry an optional `evaluation` — a portable, content-addressed CLAIM about the attested artifact's quality/confidence and how it was judged (`{ score (0..1), method (`self`/`test`/`judge`/`human`/…), checks?: [{ name, passed, note? }], evaluator? }`). Because the evaluation is part of the canonical record content, the content-hash id (and any signature) cover it — the score can't be silently edited. `evalOf(artifactHash, attestations)` returns the live claim; `evalCoverage(hashes, attestations, { threshold })` returns `{ evaluated, unevaluated, mean_score, min_score, below }` — the "which agent outputs are low-confidence / unevaluated?" audit. The OTel bridge emits `provenant.eval.score` / `provenant.eval.method` / `provenant.eval.checks_passed`. CLI: `attest --eval-score --eval-method [--eval-check name:pass …]`, `eval <file-or-hash>`, `confidence <paths…> [--threshold]`. Fills the research-validated 2026 gap — *no open protocol for attaching provenance AND evaluation scores to agent outputs*. Additive and fully backward-compatible: an attestation with no `evaluation` is byte-for-byte identical to before. Distinct from runtime outcome verification (a sibling) — this is an attested claim, auditable like coverage. Pure, deterministic, zero-dep.
